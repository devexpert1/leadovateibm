...
// The authentication guard for admin
    ...
    protected $fillable = [
        'participate_similar','user_id','course_id','syllabus_id','lesson_id','user_selection','name','user_input','activity_title','activity_time','participation_grade','hour_week','week_year'
    ];
     /**
     ...
     public static function getwithuserid2($id,$lid,$lid2){
         
         return $updateoptions = Useractivity::where('user_id',$id)->where('course_id',$lid)->where('lesson_id',$lid2)->get(); 
     }
     ...
     public static function getdatawithid($id){
        ...
        ->where('user_activities.course_id',$id)
         ->where('user_activities.user_id',Session::get('userid'))
        ->select('user_activities.*','courses.type','courses.title')
        ->get();
        return $get;
