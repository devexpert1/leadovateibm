...
class AdminController extends Controller
{
    ...
    public function logincheck(Request $request)
    {
       ...
       if($request->isMethod('post'))
      {
       ...
       if (!empty($user)) {
            ...
            Session::save(); 
            $messags['message'] = "Login successfully!!";
            $messags['erro']= 101;
            $messags['url']= '';
         }else{
	 ...
	 public function dashboard()
    {
	 ...
	 { 
	     if(session()->exists('admin'))
      {
        $userid =Session()->get('adminid');
        $data['admin'] = Admin::where(array('id'=>$userid))->first();
        $data['users'] = DB::table('users_refer')->orderby('id','desc')->get();
        return view('admin/ref_users',$data);
      }else
      {
          return Redirect('/admin');
      }
    ...
    if(!empty($exist))
 {
	...
	public function add_course(Request $request)
	{
	  ...
	  if(session()->exists('admin'))
      {
        ...
        elseif(isset($data['add_syllabus'] ) && $data['add_syllabus'] =='add_lesson')
            ...
            CURLOPT_URL => "https://iam.cloud.ibm.com/identity/token?grant_type=urn%3Aibm%3Aparams%3Aoauth%3Agrant-type%3Aapikey&apikey=at7RSy_fpRc78pcx0Hb79zW-v5IgjcRZzE7scD-BpuTY",
            ...
            "postman-token: 466d00b2-a552-08d2-95c9-f5d91b3ec7c9"
            ),
            ));
            ...
            {
            $ch = curl_init();
            $data11=file_get_contents($filedata);
            curl_setopt($ch, CURLOPT_URL,"https://s3.jp-tok.cloud-object-storage.appdomain.cloud/structenspace/$filename");
            ...
            $type='Video';
            $image=$t['file'][$i];
            $imagename =preg_replace('/[^a-zA-Z0-9-_\.]/','', $image);
            ...
            $course_data=Course::getdatawithid($data['course_id']);
            ...
            $messags['course_type']=$course_data->type;
            ...
            die;  
                ...
                elseif(isset($data['add_syllabus'] ) && $data['add_syllabus'] =='add_example')
                {
                    ...
                    {
                            if($course_data->type =='E'){
                            $query=array(
                                 "category_id"=>$t['category_id'][0],
                           // "lesson_id"=>$t['lesson_id'][0],
                     ...
                     die;  
                      ...
                      if($request->file('file'))
                        ...
                        $image = $request->file('file1');
                         $rand=rand(100,889);
                        $imagename_cover =$rand.time().'.'.$image->getClientOriginalExtension();
                        $destinationPath = public_path('/uploads');
                        $image->move($destinationPath, $imagename_cover);
        ...
        if(session()->exists('admin'))
        {
                ...
                $data = $request->all();
                  if(!empty($_POST['id']) && !empty($_POST['title']) )
                  {
                        if($request->file('file'))
    			            ...
    			            $image = $request->file('file1');
    			            $rand=rand(100,334);
    			            $imagecover =$rand.time().'.'.$image->getClientOriginalExtension();
    			            $destinationPath = public_path('/uploads');
    			            $image->move($destinationPath, $imagecover);
    			            }else
    			            {
    			               $imagecover=$_POST['image_banner_course'];
    			            }
		                    unset($data['_token']);
                            $query= array(
                            //'type'=>$data['type'],
            ...
            curl_close ($ch);
                ...
                elseif(isset($data['add_syllabus'] ) && $data['add_syllabus'] =='edit_example')
                {
                    ...
                    else if(!empty($t['lesson_id'][0]) && count($t['lesson_id']) >= '1') 
                    {
                    if(isset($t['example_description']))
                    {
                    for($i=0;$i<count($t['example_description']);$i++)
    ...
    {
        $data['options'] = Options::getoption();
            $data['page']='Stripe Settings';
          return view('admin/stripe',$data);
    }
     public function courses_purchased  (Request $request,$id)
    {
         $data['options'] = User_package::where('user_id',$id)->orderby('id','desc')->paginate('5');
        
        return view('admin/courses_purchased',$data);
