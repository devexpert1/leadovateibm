...
class Controller extends BaseController
{
	 ...
	 public function register_user(Request $request)
    {
        ...
        {
        $data=$_POST;
		if($data['captcha']!=Session::get('vercode'))
		{
		return Redirect::to('/register')->with('error', 'Captcha does not match. Please try again');
		die;
		
		}else
		{
		$pos = strpos($data['email'], "ppoet.com");
		if ($pos === false) {
        ...
        $date=date('Y-m-d');
        $time = strtotime($date);
        $final = date('Y-m-d', strtotime("+$subscription->duration day", $time));
		...
		return Redirect::to('/register')->with('error', 'Error in register. Please try again');
		}
		}
