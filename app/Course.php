...
protected $fillable = [
'type','title','image','description','course_overview','status','enroled_ids','order','banner_image','price'
];
/**
...
public static function getactiveall($id)
{
$get = DB::table('courses')
->Join('lesson', 'lesson.course_id', '=', 'courses.id')
//->leftJoin('usercourses', array('courses.id', '=', 'usercourses.course_id')
 ->leftJoin("usercourses",function($join) use ($id){
            $join->on("courses.id","=","usercourses.course_id")
                ->where("usercourses.user_id", $id);
        })
->where('courses.status','0')
->where(function($query) {
                 $query->where('usercourses.status', '0');
                 $query->orwhere('usercourses.status', NULL);
             })
