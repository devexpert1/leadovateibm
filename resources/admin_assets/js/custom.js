  ...
  function submitform1(form,btn)
{
	...
	if(form=='addterms_conditions')
	{
		var desc = CKEDITOR.instances['editor1'].getData();
		 data.append("tc_content", desc);
	}
	else if(form=='addprivacy_policy')
	{
		var desc = CKEDITOR.instances['editor1'].getData();
		 data.append("pp_content", desc);
	}else
	{
	}
...
(function($) {
    ...
    {
        var data = new FormData($('.addcourse_syllabs_form')[0]);
	...
	$(document).on('click', '.edit_lesson_syl', function () {
	     if($('.edit_lesson').valid())
      {
		...
		fileSize= fileSize+Math.round(this.files[i].size/1024/1024);
		}
		});
		if(fileSize > 5120)
		{
			$(".dg1").show();
            $(".dg1").html("Size of selected files should be less then 5GB");
            setTimeout(function(){ $('.alert-danger').hide(); }, 3000);
			return;
		}else
		{
...
$(document).on('change', '.file_video', function (e) {
        var exactSize =Math.round(this.files[0].size/1024/1024);
 ...
 if ($.inArray($(this).val().split('.').pop().toLowerCase(), fileExtension) == -1) {
    ...
    return;
}else if(exactSize >= '1024'){
    $(".dg1").html('Please upload less than 1 GB.');
    $(".dg1").show(); 
    $(this).val("");
    setTimeout(function(){ $('.dg1').hide(); }, 3000); 
    return;
