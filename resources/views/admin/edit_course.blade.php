...
.new
{
    ...
    float: left;
    width: 100%;
}form.addcourse .form-group.col-sm-2.col-2 {
    text-align: center;
}
form.addcourse .form-group img.upsave, form.addcourse .form-group img#blah {
    max-height: 80px;
    width: auto;
    margin: 0 auto;
}
form.addcourse .form-group img.upsave1, form.addcourse .form-group img#blah1 {
    max-height: 80px;
    width: auto;
    margin: 0 auto;
}
								...
								<div class="card card-shadow mb-4">
									...
									<div class="row">
										<div class="col-xl-12">
											...
											<?php 	if($course_details['type'] == 'E'){?>
												
												<li class="nav-item hide">
													<a class="nav-link hide" data-toggle="tab" href="#tab_6">Activity</a>
												</li>
												<?php } ?>
												<li class="nav-item">
											...
											<div class="tab-content">
													   ...
													   @csrf
														<div class="row">
															<div class="form-group col-sm-12 col-12">
															    ...
															    <input type="hidden" id="" name="image_banner_course" value="{{$course_details['banner_image']}}" >
														...
														<div class="row">
															<div class="form-group col-sm-10 col-10">
																<label for="exampleInputPassword10">Icon Image</label>
															...
															<div class="row">
															<div class="form-group col-sm-10 col-10">
																<label for="exampleInputPassword10">Banner Image</label>
																	<input type="file" name="file1" class="form-control form-control-square fileupload1" id="files" placeholder="">
																</div>
															<div class="form-group col-sm-2 col-2">
															   @if(empty($course_details['banner_image']))
																<img class="img-shw upsave1" src="{{url('/')}}/resources/admin_assets/images/discipline.png">
															   @else
																<img class="img-shw upsave1" src="{{url('/')}}/public/uploads/{{$course_details['banner_image']}}">
															   @endif
															   <img id="blah1" src="#" alt="your image" class="media-object" style="display:none;" />
															</div>
														</div>
														...
														<div class="row">
															<div class="form-group col-sm-12 col-12">
																...
																<label for="exampleInputPassword10">Price</label>
																<input type="text" class="form-control form-control-square" name="price" id="" value="<?=$course_details['price'];?>">
															</div>
														</div>
												...
												<?php $filename='public/uploads/'.$l['description'];  ?> 
												<?php
												if (file_exists($filename)) {
												?>
                                                <source src="{{url('/')}}/public/uploads/{{$l['description']}}" type="video/mp4">
												<?php }else{ ?>
												<source src="https://s3.jp-tok.cloud-object-storage.appdomain.cloud/structenspace/{{$l['description']}}"
												type="video/mp4">
												<?php } ?>
                                                </video>
                                       ...
                                       <input type="hidden" id="type" value="{{$course_details['type']}}">
                                        ...
                                        @endforeach
                                           ...
                                           <select class="get_lesson headings_lesson_name hide" style="display:none" >
                                        @foreach($lesson as $a)
                                        ...
                                        @endif
                                        @if($course_details['type']=='E' && count($lesson)!='0')
                                         <?php $example1=App\Example::where('course_id',$course_details['id'])->groupBy('lesson_id')->get();?>
                                      
                                        <a href="javascript:void(0);" class="float-right pull-right btn btn-info  btn-adda btn 
                                        btn-success pull-right add_new" data-head="" data-less=""
                                        data-act="" data-id="{{count($lesson)}}" 
                                        data-count="{{count($example1)}}" data-id2="101010">
                                        <i class="fa fa-plus"></i>Add</a>
                                        <input type="hidden" id="example123" value="{{count($example1)}}">
                                        @endif
                                        
                                        
                                        </h4>
                                        </div>
                                        @if($course_details['type']!='E' && count($activitys)!='0')
                                        <div class="col-md-12">
                                        <?php $i='0';?>
                                       ...
                                       <p class="not_found">No Example found</p>
                                        ...
                                        <div class="col-md-12">
                                        <?php $i='0';?>
                                         <?php $example1=App\Example::where('course_id',$course_details['id'])->orderby('id','asc')->groupBy('lesson_id')->get();?>
                                        @forelse($example1 as $e)
                                        <?php $im=$i++;?>
                                        <?php $random=mt_rand(100000, 999999);?>
                                        <div class="color_div">
                                        @if(count($lesson)!='0')
                                        <div class="row">	
                                        <div class="form-group col-sm-12 col-12">
                                        <label class="label_act">Lesson</label>
                                        <select name="title[{{$im}}][lesson_id][]" class="form-control headings_lesson_name  ">
                                        @foreach($lesson as $a)
                                        <option  
                                        @if($e->lesson_id==$a->id) 
                                        selected 
                                        ...
                                        padding-right: 0px;
                                        ">
                                        <?php $exam=App\Example::where('course_id',$course_details['id'])->where('lesson_id',$e->lesson_id)->get();?>
                                        @if(!empty($exam))
				                    	@foreach($exam as $ex)
                                        <textarea   required class="content22"
                                        id="content22" 
...
$(function() {
    ...
    $('body').on('click','.addScnt', function(){
		...
		/*	
		$( '.content8' ).last().each( function() {
        CKEDITOR.replace( this );
        } );
        $( '.content7' ).last().each( function() {
        CKEDITOR.replace( this );
        } );
        $( '.content9' ).last().each( function() {
        CKEDITOR.replace( this );
        } );*/
		//	$('.content8').last().richText();
		//	$('.content9').last().richText();
